...
EditableText._lastBlockFocusOut = 0;
...
/* EditableText.uploadedFileUrl = function (file, editorConfig, insertLink) {
  // In this function, you need to store the file online and get the url, and then, to insert a link to the file in the contenteditable:
  // insertLink(url);
} */
...
EditableText._onOpenUrlInput = function (inputElem) {
  ...
  var parentElem = range.commonAncestorContainer.parentNode;
  var parent = $(parentElem).closest('a');
  if (parent && parent[0] && parent[0].href) {
	inputElem.val(parent[0].href);
  }
  EditableText._lastBlockFocusOut = Date.now();
  // inputElem.data('edit-focus-marker', 'darkgrey');
  inputElem.focus();
}
...
Template.editable_text_widget.events({
  ...
  'mousedown a:not(.wysiwyg-container a,.editable-text a)' : function (evt) {
    evt.stopPropagation();
    window.open($(evt.currentTarget).attr('href'), '_blank');
  },
  'click a:not(.wysiwyg-container a,.editable-text a)' : function (evt) {
    evt.preventDefault();
  },
  ...
  'paste .wysiwyg' : function (evt, tmpl) {
    ...
    Meteor.defer(function () {
      ...
      Meteor.defer(function () {
          wysiwygElem[0].scrollTop = wysiwygElem[0].scrollHeight;
      });
    });
  ...
  'click .wysiwyg-container .wysiwyg-save-button, focusout .wysiwyg-container .wysiwyg' : function (evt, tmpl) {
    ...
    Meteor.defer(function () {
      ...
      if (!document.hasFocus() || (evt.type === 'focusout' && ((EditableText._blockFocusoutForWYSIWYG || ((Date.now() - EditableText._lastBlockFocusOut) < 300)) || !(typeof self.saveOnFocusout !== 'undefined' && self.saveOnFocusout || EditableText.saveOnFocusout)))) {
