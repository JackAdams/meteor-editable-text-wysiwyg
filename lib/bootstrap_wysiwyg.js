	...
	insertFiles = function (files) {
	  ...
	  $.each(files, function (idx, fileInfo) {
		...
		if (maxImageSize && /^image\//.test(fileInfo.type)) {   
		  if (fileInfo.size > (maxImageSize * 1000)) {
		  ...
		  case 'gform':
			$.when(readFileInto(fileInfo, 'text')).done(function (text) {
			  ...
			  if (fileContents && fileContents.url) {
				...
				document.execCommand('insertHTML', false, '<br /><a href="' + fileContents.url + '" class="wysiwyg-' + extension + '" target="_blank"><i class="fa">&#x' + unicodeChar + ';</i>&nbsp;' + fileInfo.name + '</a>');
			  }  // 87
			}).fail(function (e) {
			  options.fileUploadError("file-reader", e);
			});
